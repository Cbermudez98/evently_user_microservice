...
handlerContainer.bind<IUserController>(Types.USER_CONTROLLER).to(UserController);
handlerContainer.bind<IUserService>(Types.USER_SERVICE).to(UserService);
handlerContainer.bind<IJwt>(Types.JWT).to(Jwt);
handlerContainer.bind<ISchemaValidator>(Types.SCHEMA_MIDDLEWARE).to(SchemaValidator);
handlerContainer.bind<IAuthService>(Types.AUTH_SERVICE).to(AuthService);
handlerContainer.bind<IAuthController>(Types.AUTH_CONTROLLER).to(AuthController);
